<template>
  <div>
    <v-btn icon flat style="position: fixed;top: 0;right: 8px;z-index: 4;" @click="$router.back()">
      <v-icon color="white">keyboard_arrow_left</v-icon>
    </v-btn>
    <template v-if="people.length">
      <template v-if="isAllowedToSee()">
    ...
    <v-dialog v-model="dialogs.loading" max-width="150">
      <v-card>
        <v-card-text class="text-xs-center">
          <v-progress-circular indeterminate v-bind:size="50" v-bind:width="4" color="green"></v-progress-circular>
        </v-card-text>
      </v-card>
    </v-dialog>
  </div>
