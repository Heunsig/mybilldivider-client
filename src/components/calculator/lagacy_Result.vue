...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      allowToSeeReulst () {
        return this.$store.getters['base/getAllowToSeeResult']
      }
    },
    methods: {
      permission () {
        if (this.$route.name === 'tutorial' || this.allowToSeeReulst) {
          return true
        } else {
          return false
        }
      ...
      checkResult () {
        this.progressForResult = true
        this.$http.post(this.$PATH_API + 'log').then(res => {
          setTimeout(() => {
            this.progressForResult = false
            this.$store.commit('base/setAllowToSeeResult', { bool: true })
          }, 3000)
        }, err => {
          console.log(err)
        })
